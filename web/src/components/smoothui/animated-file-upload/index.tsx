...
export interface AnimatedFileUploadProps {
  accept?: string;
  /** Replaces the accepted-types line under the drop prompt. */
  hint?: string;
  className?: string;
  disabled?: boolean;
  maxSize?: number;
    ...
    (newFiles: File[]): File[] => {
      ...
      if (maxSize) {
        ...
        if (oversized.length > 0) {
          setError(
            `${oversized.length === 1 ? "That file is" : `${oversized.length} files are`} over the ${formatFileSize(maxSize)} limit. Choose a smaller file.`
          );
  ...
  return (
    <div className={cn("w-full space-y-3", className)}>
      ...
      <motion.div
        ...
        aria-label="Drop your clip here, or click to choose it"
        className={cn(
          "relative flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed px-6 py-6",
      ...
      >
        ...
        <AnimatePresence initial={false} mode="wait">
          ...
          >
            {isDragOver ? "Drop it here" : "Drop your clip here, or click to choose it"}
          </motion.p>
        </AnimatePresence>
        <p className="mt-1 text-muted-foreground text-xs">
          {hint ?? (accept ? accept.replace(/,/g, ", ") : "Any file type")}
          {!hint && maxSize ? ` \u2022 Max ${formatFileSize(maxSize)}` : null}
        </p>
      </motion.div>
