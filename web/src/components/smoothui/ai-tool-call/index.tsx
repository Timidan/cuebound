...
const AIToolCallBadge = ({
  ...
  const ringColor = (() => {
    // CueBound's kept and problem colours (hex so motion can interpolate them).
    if (status === "success") {
      return "#136c62";
    }
    if (status === "error") {
  ...
  return (
    ...
    >
      <button
        ...
        className="flex w-full cursor-pointer items-center gap-2.5 px-3 py-2 text-left pointer-coarse:min-h-11"
        disabled={!hasDetail}
